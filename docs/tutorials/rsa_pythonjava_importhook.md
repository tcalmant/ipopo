...
As part of recent work on [iPOPO RSA](https://github.com/tcalmant/ipopo), an
implementation of a Python import hook based on [PEP 302](https://peps.python.org/pep-0302/)
was created so that Java remote service bundles can resolve Python packages.
On the Java side, the [ModuleResolver](https://github.com/ECF/Py4j-RemoteServicesProvider/blob/master/bundles/org.eclipse.ecf.provider.direct/src/org/eclipse/ecf/provider/direct/ModuleResolver.java)
service interface exposes the methods called by a Python import hook to resolve
a Python module.

[Here](https://github.com/ECF/Py4j-RemoteServicesProvider/blob/master/examples/org.eclipse.ecf.examples.importhook.module/src/org/eclipse/ecf/examples/importhook/module/ExampleBundleModuleResolver.java)
is an example `ModuleResolver` service implementation for a Python package
named `foo`. Note that the `PATH_PREFIX` component property points to
[`/python-src`](https://github.com/ECF/Py4j-RemoteServicesProvider/tree/master/examples/org.eclipse.ecf.examples.importhook.module/python-src/)
inside the built bundle.

## Requirements

This tutorial sample requires Karaf 4.4.6+ on Java 17 or higher for the server
process, Python 3.10+ for the iPOPO sample application process, and iPOPO 3.x.

## Run the `foo` ModuleResolver in [Karaf 4.4.6+](https://karaf.apache.org/download)
...
## Add ECF Remote Services Features to Karaf
...
## Install the Feature Exposing the `foo.ModuleResolver` Example
...
## Start the iPOPO Import Hook Sample
