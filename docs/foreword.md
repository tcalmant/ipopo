...
Both are inspired by two popular Java technologies for the development
...
iPOPO helps developers build long-running and modular IT services in Python.

About the name, iPOPO is inspired by iPOJO, which stands for *injected
Plain Old Java Object*. With Java replaced by Python, the name became
...
required access to some OS-specific low-level methods and was initially
...
links between them, but as the project grew, it became necessary to develop a
framework to handle those various parts and link them together. To stay
consistent with the Java environment used by my employer, I decided to
develop a component model similar to iPOJO while keeping the core
concepts of OSGi.
...
license. In November 2013, iPOPO adopted the Apache Software License 2.0
with release 0.5.5.
...
After a long period without updates or user requests since 2018, iPOPO was
considered mature enough to earn its new major version: iPOPO 1.0 was released
in January 2020.

An iPOPO 2.0 branch was started by contributors in 2020, trying to bring an
...
iPOPO 1.x on older Python versions and communicating with newer deployments using
...
The Service-Oriented Architecture (SOA) consists of linking objects
through provided contracts (services) registered in a service registry.

A service is an object associated with properties describing it, including
...
The conclusion is that the parts of an application that only provide a
