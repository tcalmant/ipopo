...
def start_framework_for_advertise(state_queue):
    ...
    try:
        ...
        rsa = bc.get_service(bc.get_service_reference(
            'pelix.rsa.remoteserviceadmin', None))
        # export the hello remote service via rsa
        # with the BasicTopologyManager, this will result
        # in publish via etcd
        rsa.export_service(bc.get_service_reference(
...
class EtcdDiscoveryListenerTest(unittest.TestCase):
    ...
    def test_etcd_discover(self):
        ...
        def test_handler_1(endpoint_event, matched_filter):
            ...
            self.assertTrue(
                ee_type == EndpointEvent.ADDED or ee_type == EndpointEvent.REMOVED)
...
class EtcdDiscoveryPublishTest(unittest.TestCase):
    ...
    def _get_advertiser(self):
        self.advertiser = self._get_service(
            self._get_discovery_advertiser_sr())
    ...
    def _export_svc(self):
        ...
        self.export_reg = self._get_rsa().export_service(self.svc_reg.get_reference(), {
    ...
    def test_etcd_session(self):
        self.assertIsNotNone(self._get_advertiser(
        )._sessionid, 'etcd._sessionid is null')

    def test_etcd_client(self):
        self.assertIsNotNone(self._get_advertiser()._client,
                             'etcd._client is null')
    ...
    def test_etcd_advertise(self):
        ...
        try:
            adv._client.get(ep_key)
            self.fail(
                'endpoint={0} still advertised after being removed'.format(ed_id))
        except Exception:            # exception expected
            pass
        eps = adv.get_advertised_endpoints()
        self.assertTrue(
            len(eps) == 0, 'length of eps should be 0 and is {0}'.format(len(eps)))
    ...
    def test_etcd_advertise_content(self):
        ...
        ed_val_str = list(adv._client.get(
            adv._get_session_path() + '/' + ed_id).get_subtree())[0].value
        # decode the string into json object (dict)
        val_encoded = json.loads(ed_val_str)
        # compare the original dict with the one returned
        self.assertDictEqual(encoded_ep, val_encoded,
                             'encoded endpoints not equal')
        # also check a couple of fields
        self.assertListEqual(encoded_ep['properties'], val_encoded['properties'],
